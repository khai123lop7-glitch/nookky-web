const realNooks = [
  { src: "/media/editorial/real-hoi-an.webp", name: "Phố Vừa Lên Đèn", context: "Góc đọc sách", className: "is-tall" },
  ...
  { src: "/media/editorial/real-mien-tay.webp", name: "Sông Vừa Thức Giấc", context: "Không gian sống", className: "is-wide" },
];

export function RealNooks() {
  return (
    <section className="nk-real" aria-labelledby="real-title">
      <div className="nk-container nk-section-head nk-section-head--dark">
        <div>
          <p className="nk-eyebrow">TRONG KHÔNG GIAN THẬT</p>
          <h2 id="real-title">Nook Ký trong những căn phòng thật.</h2>
        </div>
      ...
      <div className="nk-container nk-real__grid">
        {realNooks.map((item) => (
          <figure className={`nk-real__card ${item.className}`} key={item.src}>
            <img src={item.src} alt={`${item.name} trong ${item.context.toLowerCase()}`} />
